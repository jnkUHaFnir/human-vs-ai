...
int main(int argc, char ** argv) {
    ...
    while (fgets(line, MAXLINELEN, infile)) {
        ...
        line[length-1] = '\0';
        linesptr[items] = malloc(length * sizeof(char) + 1); // Allocate memory for the line
        strcpy(linesptr[items], line); // Copy the contents of the line to the allocated memory
        printf("the input string %d is : %s \n", items, linesptr[items]);
        items++;
    }

    qsort(linesptr, items, sizeof(char*), strcmp); // Change MAXLINELEN to sizeof(char*)
