...
int hash_func(const char* hash_val) {
    int h = 0;
    for (int i = 0; hash_val[i] != '\0'; i++) {
        h = (h << 2) ^ hash_val[i];
    }
    return (unsigned int)h % HASH_SIZE; // Ensure the modulus result is positive
}
...
bool load(const char *dictionary) {
    ...
    free(string); // Free the memory allocated by %ms format specifier
    fclose(dic);
    return true;
}
