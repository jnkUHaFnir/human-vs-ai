...
int main(void) {
    ...
    for (i = 0; i < 3; i++) {
        student_t[i].name = malloc(100 * sizeof(char)); // Allocate memory for name
        student_t[i].surname = malloc(100 * sizeof(char)); // Allocate memory for surname
        student_t[i].UUN = malloc(10 * sizeof(char)); // Allocate memory for UUN
        student_t[i].department = malloc(50 * sizeof(char)); // Allocate memory for department

        if (student_t[i].name == NULL || student_t[i].surname == NULL ||
            student_t[i].UUN == NULL || student_t[i].department == NULL) {
