...
int main(void) {
    ...
    for (i = 3; i < 6; i++) {
        student_t[i].name = malloc(50 * sizeof(char)); // Allocating memory for the strings
        student_t[i].surname = malloc(50 * sizeof(char));
        student_t[i].UUN = malloc(50 * sizeof(char));
        student_t[i].department = malloc(50 * sizeof(char));
