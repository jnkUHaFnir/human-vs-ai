...
int main(void) {
    ...
    for (i = 3; i < 6; i++) {
        student_t[i].name = malloc(100 * sizeof(char)); // Allocating memory for name
        student_t[i].surname = malloc(100 * sizeof(char)); // Allocating memory for surname
        student_t[i].UUN = malloc(10 * sizeof(char)); // Allocating memory for UUN
        
        printf("Enter your name: ");
        scanf(" %99s", student_t[i].name); // Limiting input to prevent buffer overflow
        printf("Enter your surname: ");
        scanf(" %99s", student_t[i].surname);
        printf("Enter your UUN: ");
        scanf(" %9s", student_t[i].UUN);
        printf("Enter your department: ");
        scanf(" %s", student_t[i].department); // No need to allocate memory for department as it's a string literal
