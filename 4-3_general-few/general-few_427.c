...
void main() {
    ...
    dummies = (Dummy**) malloc(sizeof(Dummy*)); // Allocate memory for the pointer to pointers
    if (!dummies) {
        exit(1); // allocation unsuccessful
    }
    ...
    free(*dummies); // Free memory allocated for dummies
    free(dummies); // Free memory allocated for dummies pointer
}
