...
int main(void) {
  ...
  // Access and use the loaded data here as needed
...
int Load(SPerson Archive[], unsigned int *ne) {
  ...
  while (fscanf(f, "%19s %19s %19s %19s", s[0], s[1], s[2], s[3]) == 4) {
    ...
    Archive[i].name = (char*)malloc(strlen(s[1]) + 1);
    strcpy(Archive[i].name, s[1]);
    Archive[i].surname = (char*)malloc(strlen(s[2]) + 1);
    strcpy(Archive[i].surname, s[2]);
    Archive[i].telephone = (char*)malloc(strlen(s[3]) + 1);
