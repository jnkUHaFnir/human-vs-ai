...
int main () {
    ...
    int number;
    char letter[2]; // letter[2] for string termination 
    if (file == NULL) {
    ...
    sort(number, arr); // changed this line 

    printf("\nNumber of structures with letter \"%s\" : %d\n", letter, count(number, arr, letter));

    for(int i=0; i<number && i<20; i++) {
        printf("Structure %d:\nfield int:%d\nfield char:%c\nfield float:%f\n\n", i+1, (*arr)[i]->fieldint, (*arr)[i]->fieldchar, (*arr)[i]->fieldfloat);
    }

    deleting(number, arr); // changed this line 
...
Foo **initialize(int N) { 
    Foo **array;
    array = (Foo **)malloc(sizeof(*array) * N);
    srand(time(NULL));    
