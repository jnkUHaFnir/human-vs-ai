    ...
    { 
        ...
        while (GetQueuedCompletionStatus(hCompletionPort, &numBytes, &CompletionKey, (LPOVERLAPPED*)&PerIoData, INFINITE))
        {
            ...
            else
            {
                ...
                std::cout << std::string((char*)pPerIoData->wsaBuf.buf, numBytes);
    
                PerIoData->wsaBuf.len = sizeof(PerIoData->Buffer); 
    
                if (WSARecv(PerIoData->Socket, &(PerIoData->wsaBuf), 1, &(PerIoData->dwBytesTransferred), NULL, &(PerIoData->wsaOverlapped), NULL) == 0)
                    continue;
    ...
    { 
        ...
        if (WSARecv(Socket, &(pPerIoData->wsaBuf), 1, &(pPerIoData->BytesTransferred), NULL, &(pPerIoData->wsaOverlapped), NULL) == SOCKET_ERROR)
        {
