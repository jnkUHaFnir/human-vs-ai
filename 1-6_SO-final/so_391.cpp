    ...
    static DWORD WINAPI ServerWorkerThread(LPVOID lpParameter)
    {
        ...
        while (GetQueuedCompletionStatus(hCompletionPort, &numBytes, &CompletionKey, (LPOVERLAPPED*)&PerIoData, INFINITE))    
        {
            if (!PerIoData)
                continue;
            ...
            {
                PerIoData->wsaBuf.len = strlen((char*)PerIoData->Buffer);
    
                if (WSASend(PerIoData->Socket, &(PerIoData->wsaBuf), 1, &(PerIoData->dwBytesTransferred), NULL, &(PerIoData->wsaOverlapped), NULL) == 0)
                    continue;
    
                if (WSAGetLastError() == WS_IO_PENDING)
                    continue;
            }
    ...
    {  
        ...
        pPerIoData->wsaBuf.len = strlen((char*)pPerIoData->Buffer);  
          
        if (WSASend(acceptSocket, &(pPerIoData->wsaBuf), 1, &(pPerIoData->BytesTransferred), 0, &(pPerIoData->Overlapped), NULL) == SOCKET_ERROR)
        {
            if (WSAGetLastError() != WS_IO_PENDING)
            {
                delete pPerIoData;
