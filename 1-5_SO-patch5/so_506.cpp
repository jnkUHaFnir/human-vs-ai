    ...
    // A wrapper class to enable std::vector to use ```reallocate```.
    template <class Alloc>
    struct Alloc_wrapper_for_stdvector {
        template <class T>
        struct type: protected Alloc {
            using value_type = T;
            using pointer = value_type*;
    
            void *prev = nullptr;
            ~type() noexcept {
                this->Alloc::deallocate(prev);
            }
            pointer allocate(std::size_t len) {
                if (prev)
                    return (pointer) this->Alloc::reallocate(prev, len);
                else
                    return (pointer) this->Alloc::allocate(len);
            }
