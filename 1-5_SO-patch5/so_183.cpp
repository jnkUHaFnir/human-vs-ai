    ...
    std::string getIostreamVFSName() {
    	...
    	// all the functions get in fact a pointer to an sqlite3_file object
    	...
    	struct Functions {
    		...
    		static int xOpen(sqlite3_vfs*, const char* zName, sqlite3_file* fileBase, int flags, int *pOutFlags) {
    			...
    			if (strlen(zName) != sizeof(void*) * 2) {
    				...
    				new (&fileData->stream) std::shared_ptr<std::iostream>(*reinterpret_cast<std::shared_ptr<std::iostream>*>(sharedPtrAddress));
    			}
    		...
    		static int xRead(sqlite3_file* fileBase, void* buffer, int quantity, sqlite3_int64 offset) {
    			...
    			std::istream::sentry sentry(*fileData->stream);
    			//assert(sentry);	// fails all the time, don't know why (I'm not so familiar with sentries)
    
    			// we try to seek to the offset we want to read
    			fileData->stream->seekg(offset);
    			...
    			// if it fails we return an error this time, because this is not a normal behavior
    			if (fileData->stream->fail()) {
    				fileData->stream->clear();
    				memset(static_cast<char*>(buffer), 0, quantity);
    				return SQLITE_IOERR;
    		...
    		static int xTruncate(sqlite3_file* fileBase, sqlite3_int64 size) {
    			...
    			// for the moment we'll just hope that SQLite does not call this function
    			assert(false);	// NYI
    			std::terminate();
    			return SQLITE_OK;
    		}
    		...
    		static int xLock(sqlite3_file* fileBase, int flags) {
    			if (flags == SQLITE_LOCK_NONE)
    				return SQLITE_OK;
    
    			auto fileData = static_cast<File*>(fileBase);
    			assert(fileData->lockLevel < std::numeric_limits<decltype(fileData->lockLevel)>::max());
    			++fileData->lockLevel;	// increasing the lock level
    			return SQLITE_OK;
    		}
    
    		static int xUnlock(sqlite3_file* fileBase, int flags) {
    			if (flags == SQLITE_LOCK_NONE)
    				return SQLITE_OK;
    
    			auto fileData = static_cast<File*>(fileBase);
    			assert(fileData->lockLevel >= 1);	// this happened to fail sometimes in case of i/o errors, but it's probably a neglectable bug in SQLite
    			--fileData->lockLevel;		// decreasing the lock level
    			return SQLITE_OK;
    		}
    		...
    		static int xFileControl(sqlite3_file* fileBase, int op, void* pArg) {
    			...
    			switch(op) {
    				...
    				case SQLITE_GET_LOCKPROXYFILE:		assert(false); std::terminate();	// NYI due to lack of documentation
    				case SQLITE_SET_LOCKPROXYFILE:		assert(false); std::terminate();	// NYI due to lack of documentation
    				case SQLITE_LAST_ERRNO:				assert(false); std::terminate();	// NYI due to lack of documentation
    			}
    	...
    	/*readStructure.xCurrentTime = &Functions::xOpen;
    	readStructure.xGetLastError = &Functions::xOpen;
    	readStructure.xCurrentTimeInt64 = &Functions::xOpen;*/
