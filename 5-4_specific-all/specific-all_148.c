...
void loadBookName(char* filename, char* authorName[55], char* bookName[55])
{
    ...
    for (i = 0; i < 55; i++)
    {
        ...
        token = strtok(string_array, comma); // Extract author name
        authorName[i] = (char*)malloc(strlen(token) + 1); // Allocate memory for author name
        strcpy(authorName[i], token); // Copy author name into authorName array
        
        token = strtok(NULL, comma); // Extract book title
        bookName[i] = (char*)malloc(strlen(token) + 1); // Allocate memory for book title
        strcpy(bookName[i], token); // Copy book title into bookName array
    }
