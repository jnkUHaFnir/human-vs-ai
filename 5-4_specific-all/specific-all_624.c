...
int main(){
    ...
    for(;;){
        ...
        int bytesRec = recv(clientSocket, buf, BUFSIZE, 0);
        buf[bytesRec] = '\0'; // Ensure buffer ends with null terminator
        printf("result %s\n", buf);
        ...
        close(clientSocket); // Close the client socket after sending the response
    }
...
int main(){
    ...
    char serverReply[1024]; // Changed to character buffer to store server response
    ...
    for(;;){
        ...
        if(bytesRec < 0){
            ...
            serverReply[bytesRec] = '\0'; // Ensure buffer ends with null terminator
            printf("%s\n", serverReply);
        }
    }
