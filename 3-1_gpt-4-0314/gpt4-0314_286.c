...
char text[1000] = "{\"success\": true,\"language\": \"en\",\"action\":"
                " \"GetServiceList\",\"version\": 1,\"data\": {\"service_list\" :[{\"id\":"
                "4806,\"provider_id\": 581,\"name\": \"Bill Payment (MG SC"
...
void service_data_func(char text[]) {
    ...
    while (1) { // while ALL
        ...
        } else if (strstr(text + i, "service_charge_list")) {
            i += strlen("service_charge_list");
            while (1) { //while ALL
                if (i >= len) {
                    break;
                }
                if (text[i] == 'f' && text[i + 1] == 'r'&&text[i + 2] == 'o'&& text[i + 3] == 'm') {
                    while (1) { //while from
                        ...
                        if (text[i] == ':') {
                            ...
                            memset(name, 0, sizeof(name));
                            if (text[i] == '}') {
                                // Move out of the service_charge_list
                                while (text[i] != ']') {
                                    i++;
                                }
                                menunum_charge = 0;
