...
void readFromFile() {
    ...
    while (currentSz <= maxSz) {
        float *fileArr = (float*)malloc(currentSz * sizeof(float)); // Dynamically allocate memory
        ...
        // heapSort(fileArr, currentSz); // Uncomment this line after fixing the memory issue
        diff = clock() - start;
        ...
        free(fileArr); // Free dynamically allocated memory
