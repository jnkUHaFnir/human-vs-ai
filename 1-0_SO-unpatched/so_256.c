    void parse_http_headers(int s, FILE * fp)
    {
       ...
       // updating the buffer size since it is vital that a single
       // line is not longer than the buffer and reducing the calls
       // to read() improves efficiency.
       isnheader = 0;
       len       = 0;
    
       // read next chunk from socket
       while((readed = recv(s, &buffer[len], (1023-len), 0)) > 0)
       {
          // process headers
          if (isnheader == 0)
          {
